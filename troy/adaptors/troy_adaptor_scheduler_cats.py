...
class adaptor (aBase) :
    ...
    def __init__ (self) :
        # duh!
        self.name     = 'troy_adaptor_scheduler_cats'

        # The registry maps api interface classes to the adaptor classes 
        # implementing them:
        self.registry = { 'Scheduler' : 'scheduler_cats' }
...
########################################################################
class scheduler_cats (troy.interface.iScheduler) :
    ...
    def __init__ (self, api, adaptor) :
        ...
        if 'policy' in self.api._idata :
            if self.api._idata['policy'] != 'Cats' :
    ...
    def schedule (self, t, ud) :
        ...
        pilots = []
        print "MEOW!!!  You're in the cat scheduler!!!"
        exit(0)
