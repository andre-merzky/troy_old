...
class ComputeScheduler_ (Base) :
    ...
    Troy.  In particular, it will schedule compute units over a set of compute
    ...
    def schedule (self, cus, cud) :
        ...
        A compute unit description is used to instantiate a CU on the resource
        managed by a specific compute pilot.  The scheduler can operate over all
        pilot services known to the CUS, and their pilots.
            
        If the scheduler cannot find suitable resources for the requested CU,
        a BadParameter exception is raised.  Not raising this exception is not
        a guarantee that the CU will in fact be (able to be) enacted -- in that
        case, the returned CU will later be moved to Failed state.
            
        On success, the returned CU is in Pending state (or moved into any state
        downstream from Pending).

        schedule() will honor all attributes set on the cud.  Attributes which
        are not explicitly set are interpreted as having default values (see
        documentation of CUD), or, where default values are not specified, are
        ignored.
        """
        return self.engine_.call ('ComputeScheduler', 'schedule', self, cus, cud)
