...
class DataPilotService (Base) :
    ...
    """  DataPilotService (DPS)
        ...
        creates and manages L{DataPilot} instances within that framework, and
        can scheduler L{DataUnit}s amongst those pilots.

        The class is stateful, and instances are identified by an url, and can
        be reconnected to.  A DPS can be added to a L{DataUnitService}, whose
        scheduler will then be able to utilize the DPS's pilots for
        L{DataUnit} enaction.
    ...
    def submit_pilot (self, dpd) :
        """ Create a DataPilot.
            ...
            DPS, a BadParameter exception is raised.  Not raising this exception
            is, however, not a guarantee that the DP will in fact be (able to
            be) executed -- in that case, the returned DP will be moved to
            Failed state.
    ...
    def list_pilots (self) :
        """ list managed L{DataPilot}s.
            ...
            this DPS instance.  The list may be incomplete, and may not include
            pilots created by the DPS.  There is no guarantee that pilots in the
    ...
    def submit_unit (self, dud) :
        """ Submit a DU to this DataPilotService.
            ...
            any of its data pilots.  If no suitable pilot is found,
            a L{Error.BadParameter} exception is raised.  Not raising this
            exception is not a guarantee that the DU will in fact be (able to
    ...
    def list_units (self) :
        """ list managed L{DataUnit}s.
            ...
            this DPS instance.  The list may be incomplete, and may not include
            units created by the DPS.  There is no guarantee that units in the
            returned list can in fact be reconnected to.  Also, an inclusion in
